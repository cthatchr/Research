...
def distribute(stations, priority, dist_constraint=False):
    ...
    dl.fill(stations, target, dist_constraint)
    dlist = dl.distList
    # print(len(dlist))
    ...
    rrst = get_reroute_station(dlist, target, dist_constraint, priority)  # assign reroute station for target station
    if rrst is None:
        return 0
...
def get_reroute_station(dlist, target, distance_constraint, priority):  # gets rr station for target station
    rr = None
    # print(priority)
    if target.is_deficit():  # when target station has a deficit, get rrst to reroute user to target
            ...
            if s.has_rr_user():  # check to make sure station has a rerouteable user
                if ((s.getdiff() > target.getdiff()) and (s.is_surplus())):  #  and (y.dist < distance_constraint)
                    ...
                    # this is where different priority checks would be placed
                        ...
                        if priority == 2:  # distance only, use smallest value
                            # print('in 3', priority)
                            if y.priority < rr.priority:  # if iterations priority > best so far, set as new best
                                rr = y
                        else:
                            # print('in else',priority)
                            if y.priority > rr.priority:  # if iterations priority > best so far, set as new best
                                rr = y
        ...
        for y in dlist:
            s = y.station
            if ((s.getdiff() < target.getdiff()) and (s.is_deficit())):  # diff must be less than target//diff and (y.dist < distance_constraint)
                if rr is None:
                    rr = y
                # this is where different priority checks would be placed
                else:
